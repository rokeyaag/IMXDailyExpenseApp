...
<body>
  <h1>${t("reportTitle")}</h1>
  <p class="subtitle">${t("period")}: ${formatDate(period.start_date)} — ${formatDate(period.end_date)}</p>

  <div class="summary">
    <div class="summary-item">
      <div class="summary-label">${t("income").toUpperCase()}</div>
      <div class="summary-value">Tk ${summary.total_income.toFixed(2)}</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">${t("expense").toUpperCase()}</div>
      <div class="summary-value">Tk ${summary.total_expense.toFixed(2)}</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">${t("balance").toUpperCase()}</div>
  ...
  ${category_breakdown.length > 0 ? `
    ...
    <table>
      <tr><th>${t("category")}</th><th style="text-align:right;">${t("total")}</th></tr>
      ${catRows}
    </table>
  ` : ""}

  <h2>${t("transactions")} (${summary.transaction_count})</h2>
  <table>
    <tr>
      <th>${t("date")}</th>
      <th>${t("note")}</th>
  ...
  <div class="footer">
    ${t("generatedBy")} ${formatDate(new Date().toISOString())}
  </div>
</body>
</html>
    `;
