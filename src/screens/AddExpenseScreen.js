...
export default function AddExpenseScreen({ navigation, route }) {
  ...
  const handleSave = async () => {
    if (!amount) { Alert.alert("Error", "Amount ???"); return; }
    setLoading(true);
    try {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.typeRow}>
        <TouchableOpacity
          ...
          <Text style={[styles.typeBtnText, type === "expense" && styles.typeBtnTextActive]}>???</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.typeBtn, type === "income" && styles.typeIncome]}
          onPress={() => setType("income")}>
          <Text style={[styles.typeBtnText, type === "income" && styles.typeBtnTextActive]}>???</Text>
        </TouchableOpacity>
      </View>
